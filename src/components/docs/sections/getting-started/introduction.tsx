...
import { Copy, Code2, Package, ShieldCheck, Zap, Layers, GitBranch, Server, Settings, CheckCircle2 } from "lucide-react";

const IconMap: Record<string, any> = {
  Copy, Code2, Package, ShieldCheck, Zap, Layers, GitBranch, Server, Settings, CheckCircle2
};
...
export function Introduction() {
  const [data] = useState({
    title: "Introduction",
    features: [
      { title: "Universal Compatibility", description: "SnapJSX works with both React/JSX and pure HTML/CSS, giving you total freedom.", icon: "Copy" },
    ...
    philosophy: {
      ...
      points: [
        { title: "Full Ownership", description: "You own the code, not a package manager" },
        { title: "Zero Dependencies", description: "No security vulnerabilities from third parties" },
        { title: "Fully Customizable", description: "Modify without fighting framework abstractions" },
        { title: "Minimal Bundle", description: "Ship only what you use, nothing more" }
      ],
    },
    whySnapJSX: {
      title: "Why SnapJSX?",
    ...
    environments: {
        ...
        description: "SnapJSX is platform-agnostic. It works anywhere Tailwind CSS works.",
        items: ["Next.js (App & Pages Router)", "Vite + React", "Remix", "Astro", "Laravel + Inertia"]
    },
    principles: {
        title: "Engineering Principles",
        items: [
            { title: "Predictable", description: "Standard component architecture that follows React best practices." },
            { title: "Tree-Shakeable", description: "Because you copy-paste the source, your bundle size is exactly the size of the components you use." },
            { title: "Extensible", description: "Use hooks, context, or custom props without fighting restrictive APIs." }
        ]
    },
  ...
  const { features, philosophy, whySnapJSX, environments, principles, credits } = data;
  ...
  return (
    <div className="w-full space-y-16">
      ...
      <div className="rounded-3xl border border-border p-8 md:p-12 bg-accent/10 space-y-8">
        ...
        <p className="text-lg text-muted-foreground max-w-3xl">{philosophy.description}</p>
      ...
      <section className="space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {whySnapJSX.points.map((point) => {
             ...
             return (
              <div key={point.title} className="p-6 rounded-2xl border border-border bg-card/30 space-y-2">
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
                {environments.items.map(i => (
                    <span key={i} className="px-3 py-1.5 rounded-full border border-border bg-muted text-xs font-bold">{i}</span>
                ))}
            </div>
        </div>
        <div className="space-y-6">
