    ...
    <div className="w-full space-y-20">
      ...
      <div className="space-y-16">
        {strategies.map((strategy, idx) => (
          <section key={strategy.title} className="grid gap-8 md:gap-10 grid-cols-1 xl:grid-cols-2">
              ...
              {strategy.example && (
                <div className="space-y-4">
                  <div className="rounded-xl md:rounded-2xl border border-border bg-card/30 p-4 md:p-6 space-y-4">
                     ...
                     <code className="text-[10px] md:text-xs font-mono text-muted-foreground block bg-muted/50 p-2 md:p-3 rounded-lg overflow-x-auto">{strategy.example.code}</code>
                  </div>
                  <div className="rounded-xl md:rounded-2xl border border-primary/30 bg-primary/5 p-4 md:p-6 space-y-4 relative overflow-hidden group">
                     ...
                     <code className="text-[10px] md:text-xs font-mono text-primary block bg-primary/10 p-2 md:p-3 rounded-lg font-bold overflow-x-auto">{strategy.updated?.code}</code>
                  </div>
                </div>
              )}
      ...
      <section className="relative rounded-3xl md:rounded-[3rem] overflow-hidden border border-border bg-card/30">
         ...
         <div className="grid grid-cols-1 lg:grid-cols-2">
            <div className="p-8 md:p-12 space-y-6">
               <h2 className="text-3xl md:text-4xl font-black tracking-tight">{theming.title}</h2>
               <p className="text-base md:text-lg text-muted-foreground leading-relaxed">
                  {theming.description}
               </p>
               <div className="flex flex-wrap gap-2 md:gap-4 pt-2 md:pt-4">
                  <div className="px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-border bg-background text-[10px] md:text-xs font-bold">globals.css</div>
                  <div className="px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-border bg-background text-[10px] md:text-xs font-bold">Tailwind variables</div>
                  <div className="px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-border bg-background text-[10px] md:text-xs font-bold">CSS custom properties</div>
               </div>
            </div>
